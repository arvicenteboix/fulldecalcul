...
:::caution
Una de les limitacions que tindríem en este cas será el de utilitzar dates o el que hem dit de copiar i enganxar una línia, ja que, encara que li diguem que ens escriga la data actual (Ctrl+,), la gravadora no ho detecta. Pero podries posar altre camp amb la data de la factura.
:::
...
Ens apreixerà la següent barra de ferramentes. En esta ens hem de fixar en un botó per a Activar el mode edició, en el qual podem afegir controls, però no podem fer ús dels controls. Quan desactivem el mode edició podre fer ús del controls, com per exemple apretar el botó que hem afegit.

![Barra de control de formulari](img/21.png){ width=60% }
...
![Escollim macro](img/25.png){ width=60% }

:::caution
Recorda que per a que funcione el botó has d'eixir del mode d'edició polsant:

![Eixir mode edició del formulari](img/26.png){ width=60% }
